...
;;; (c)1996-2000 by HIROSE Yuuji [yuuji@gentei.org]
;;; $Id: xcite.el,v 1.33 2000/04/21 13:48:29 yuuji Exp $
;;; Last modified Fri Apr 21 22:47:34 2000 on firestorm
;;; Update count: 387

;;[News in 1.33]
;;	
;;	Semi-gnus対応のfix(by 土屋さん)
;;	
;;[Commentary]
;;
...
;;[Warning]
;;
;;	You can use this package freely unless you don't cite your own
;;	article with a first-person citation prefix, as `me' which will
;;	lose information in the future citation.
;;
...
;;	を指定できます。以下に、mewを用いている場合の X-cite-me ヘッダの
;;	自動挿入設定を示します。
;;
;;		;;for Mew
...
;;
;;				Jul. 1999 広瀬雄二 [yuuji@gentei.org]
;;

;;;
...
(defvar xcite:first-second-preson
  "^\\(私\\|[あわ]たく?し\\|僕\\|ぼく\\(ちん\\)?\\|俺\\|おれ\\|me\\|わし\\|あっし\\|おいら\\|拙者?\\|自分\\|己\\|朕\\|余\\|あなた\\|君\\|おまえ\\|[きち]み\\)$"
...
(defun xcite (arg)
  ...
  (let ((c (downcase (read-char))))
    (cond
     ...
     ((= c ?w) (xcite-cite t nil nil arg))
     ((= c ?a) (xcite-cite 'append nil nil arg))
     ((= c ?p) (xcite-cite 'prepend nil nil arg))
     ((= c ?i) (xcite/prefix))
...
(defun minibuffer-selection-forward (arg)
  ...
  (interactive "p")
  (let ((list minibuffer-completion-table) (current (buffer-string)) (i 0))
    (while list
    ...
    (erase-buffer)
    (insert (car (nth i minibuffer-completion-table)))))
...
should go."
  ...
  (let ((sw (selected-window)) (b (make-marker)) (e (make-marker))
    ...
    (setq x-cite-me (xcite-get-field "X-cite-me:"))
  ...
  (let ((curw (selected-window)) b)
    (or
     ...
     (and (eq major-mode 'mew-draft-mode) ;Support mew
	  (fboundp 'mew-cache-hit) (fboundp 'mew-current-get)
	  (save-excursion
...
(defun xcite-match-string (n &optional m)
  (if (match-beginning n)
      (buffer-substring (match-beginning n) (match-end (or m n)))
    nil))
...
(defun xcite-get-field (field &optional default-value)
  ...
  (let ((case-fold-search nil) fptn value)
...
(provide 'xcite)
(defconst xcite:revision "$Revision: 1.33 $"
